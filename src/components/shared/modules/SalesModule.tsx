...
import { Plus, FileText, CheckCircle2, DollarSign, X, Upload, History, FileSpreadsheet, Printer, Download, Table, LayoutGrid, Lock, Unlock } from 'lucide-react';
...
export default function SalesModule({ isMobile }: { isMobile?: boolean }) {
  ...
  const columns: Column<any>[] = [
    ...
    },
    {
      ...
      cell: (o) => {
        const data = parseNotes(o.notes);
        return <span className="text-sm text-gray-900 dark:text-gray-100">{data.productionCost ? formatCurrency(Number(data.productionCost)) : '-'}</span>;
      },
    },
    {
      header: 'Selling Cost',
      ...
      cell: (o) => {
        const data = parseNotes(o.notes);
        return <span className="text-sm text-gray-900 dark:text-gray-100">{data.sellingCost ? formatCurrency(Number(data.sellingCost)) : (o.items?.[0]?.unitPrice ? formatCurrency(o.items[0].unitPrice) : '-')}</span>;
      },
    },
    {
      header: 'Margin %',
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between gap-3 mb-2">
          ...
          <div className="flex shrink-0 gap-2">
            <button
              ...
              className="rounded-xl bg-blue-600 p-2 text-white shadow-sm active:bg-blue-700"
            >
      ...
      {(!isMobile || viewMode === 'table') ? (
        <div className={cn("bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden", !isMobile && "border-none shadow-none bg-transparent")}>
          <DataTable 
      ...
      {/* Create Order Modal */}
      {isCreateOpen && (
        <div className={cn("fixed inset-0 z-50 flex overflow-y-auto", isMobile ? "flex-col bg-gray-50 dark:bg-gray-950 pt-4 pb-20 px-4" : "items-center justify-center bg-black/40 p-4 backdrop-blur-sm")}>
          <div className={cn("w-full bg-white dark:bg-gray-900", isMobile ? "flex-1 border-0 shadow-none" : "max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl border border-gray-200 p-6 shadow-2xl dark:border-gray-800")}>
            <div className={cn("flex justify-between items-center", isMobile ? "mb-6" : "mb-4")}>
              <h2 className={cn("font-bold text-gray-900 dark:text-white", isMobile ? "text-xl" : "text-lg")}>
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-4">
              ...
              {/* Product Info */}
              <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-200 dark:border-gray-800 space-y-4">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Product & Pricing</h3>
                
                <div>
                  <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300">Select Product *</label>
                  <SearchableSelect
                    required
                    value={items[0]?.productId || ''}
                    onChange={(val) => {
                      const newItems = [...(items || [{}])];
                      newItems[0] = { ...newItems[0], productId: val };
                      setItems(newItems);
                      ...
                      if (selectedProduct) {
                        ...
                        newItems[0].unitPrice = selectedProduct.mrp || 0;
                        setItems(newItems);
                      }
                    }}
                    placeholder="-- Choose Product --"
                    options={products.map((p: any) => ({ value: p.id, label: `${p.name} (${p.sku || 'No SKU'})` }))}
                  />
                </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300">Order Status</label>
                <select
              ...
              <div className="flex justify-end gap-3 pt-6 border-t border-gray-100 dark:border-gray-800">
                <button
                  type="button"
                  onClick={() => setIsCreateOpen(false)}
                  className="rounded-xl px-5 py-2.5 text-sm font-semibold text-gray-600 hover:bg-gray-100 transition-colors dark:text-gray-300 dark:hover:bg-gray-800"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="rounded-xl bg-blue-600 px-6 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 transition-colors"
                >
